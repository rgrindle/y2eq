from evaluate_many_x import evaluate
...
def train(num_epochs, train_loader, valid_loader,
    ...
    for epoch in range(num_epochs):
        ...
        if valid_loss <= best_valid_loss:
            ...
            torch.save(model.state_dict(), os.path.join(save_loc, 'xy2eq{}.pt'.format(save_end_name)))
            torch.save(optimizer.state_dict(), os.path.join(save_loc, 'xy2eq_optimizer{}.pt'.format(save_end_name)))
            print('saving')
    ...
    pd.DataFrame(history.values()).T.to_csv(os.path.join(save_loc, 'xy2eq_train_history{}.csv'.format(save_end_name)), index=False, header=list(history.keys()))
...
def train_one_epoch(model, iterator, optimizer, criterion,
    ...
    for i, batch in enumerate(iterator):
        ...
        print(src.shape)
        print(torch.min(trg), torch.max(trg))
