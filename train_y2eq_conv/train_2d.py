...
TODO:
"""
from evaluate_2d import evaluate
...
def train(num_epochs, train_loader, valid_loader,
    ...
    for epoch in range(num_epochs):
        ...
        if valid_loss <= best_valid_loss:
            ...
            torch.save(model.state_dict(), os.path.join(save_loc, 'y2eq_2d{}.pt'.format(save_end_name)))
            torch.save(optimizer.state_dict(), os.path.join(save_loc, 'y2eq_2d_optimizer{}.pt'.format(save_end_name)))
            print('saving')
    ...
    pd.DataFrame(history.values()).T.to_csv(os.path.join(save_loc, 'y2eq_2d_train_history{}.csv'.format(save_end_name)), index=False, header=list(history.keys()))
