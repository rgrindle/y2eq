...
def rebuild_dataset_with_x(eq_list, other_dataset_inputs=None):
...
if __name__ == '__main__':
    ...
    for dataset_type in ['train', 'test']:
        ...
        dataset_parts = rebuild_dataset_with_x(eq_list, other_dataset_inputs)

        dataset = TensorDataset(torch.Tensor(dataset_parts[0]), torch.LongTensor(dataset_parts[1]))
        torch.save(dataset, os.path.join(dataset_path, 'dataset'+dataset_name+'_1000fixed.pt'))
