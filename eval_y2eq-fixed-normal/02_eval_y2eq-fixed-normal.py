...
with open('00_y_fixed_list.json', 'r') as json_file:
    y_list = json.load(json_file)

with open('00_y_ext_fixed_list.json', 'r') as json_file:
...
rmse_int_list = []
rmse_ext_list = []
for i, (ff, y, y_ext) in enumerate(zip(ff_list, y_list, y_ext_list)):
    if pd.isnull(ff):
        rmse_int = float('inf')
        rmse_ext = float('inf')
    elif is_eq_valid(ff):
