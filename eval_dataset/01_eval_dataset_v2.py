...
for i, (ff, y, x, y_ext) in enumerate(zip(ff_list, y_list, x_list, y_ext_list)):
    if pd.isnull(ff):
        ...
        eq = EquationInfix(ff, x=np.array(x))
        eq.fit(y)
    # elif is_eq_valid(ff):
    #     ic(ff)
    ...
    #     coeffs, rmse_int = regression_cmaes(f_hat, y, num_coeffs, np.array(x))
        ...
        y_int_pred_norm = normalize(eq.f(c=eq.coeffs, x=np.array(x)), true_min_, true_scale)

        y_ext_true_norm = normalize(y_ext, true_min_, true_scale)
        y_ext_pred_norm = normalize(eq.f(c=eq.coeffs, x=x_ext), true_min_, true_scale)
